...
int find_part(struct part *inve, int number);
...
{
  char code;
  struct part *inventory = (struct part*)calloc(inventory_sz, sizeof(struct part));
...
void insert(struct part* inventory)
{
  ...
  if (num_parts == MAX_PARTS) {
    ...
    /* return; */
      //realloc arr for twice se
      inventory = realloc(inventory, sizeof(struct part) * inventory_sz * 2);
      inventory_sz *= 2;
  }
...
int compare(const void* part1, const void* part2)
{
    return (*(struct part*)part1).number - (*(struct part*)part2).number;
}
